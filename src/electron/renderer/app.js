...
function syncHubModeUi() {
  ...
  renderSidePulseStatus();
}

function renderSidePulseStatus() {
  if (!els.sidepulseEnabledInput || !els.sidepulseSocketInput || !els.sidepulseSocketRow || !els.sidepulseStatus) return;
  const status = state.settings?.sidepulseStatus || {};
  const enabled = state.settings?.sidepulseEnabled === true;
  els.sidepulseEnabledInput.checked = enabled;
  els.sidepulseEnabledInput.disabled = status.available === false;
  els.sidepulseSocketInput.value = state.settings?.sidepulseSocketPath || status.socketPath || '';
  els.sidepulseSocketRow.classList.toggle('hidden', !enabled);
  els.sidepulseStatus.hidden = false;
  els.sidepulseStatus.className = `hub-status${enabled && status.available !== false ? ' ok' : ''}`;
  if (status.available === false) els.sidepulseStatus.textContent = t('settings.sidepulse.unavailable');
  else if (!enabled) els.sidepulseStatus.textContent = t('settings.sidepulse.disabled');
  else els.sidepulseStatus.textContent = t('settings.sidepulse.enabled', { pending: Number(status.pending || 0) });
}
...
els.sidepulseEnabledInput?.addEventListener('change', async () => {
  await saveSettings({ sidepulseEnabled: els.sidepulseEnabledInput.checked });
});
els.sidepulseSocketInput?.addEventListener('change', async () => {
  await saveSettings({ sidepulseSocketPath: els.sidepulseSocketInput.value.trim() });
});
